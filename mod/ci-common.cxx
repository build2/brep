...
namespace brep
{
          ...
          tenant_service&& service) const
  {
    ...
    // Delay the first load attempt for 5 seconds (see mod-build-task.cxx for
    // details).
    //
    build_tenant t (move (request_id),
                    move (service),
                    system_clock::now () - chrono::seconds (30 - 5));

    {
      ...
      // Similar to brep-load, cleanup the being (re-)created and the empty
      // tenants.
      //
      cstrings ts ({t.id.c_str (), ""});
