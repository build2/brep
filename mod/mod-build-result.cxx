...
handle (request& rq, response&)
{
  ...
  //
  // Note that we will release it prior to any time-consuming operations, such
  // as HTTP requests, and re-acquire it again afterwards, if required.
  //
  ...
  if (tsq != nullptr)
  {
    ...
    // Release the database connection since build_queued() notification is
    // likely to perform the HTTP request.
    //
    conn.reset ();
  ...
  if (tsb != nullptr)
  {
    ...
    // Release the database connection since build_built() notification is
    // likely to perform the HTTP request.
    //
    conn.reset ();
