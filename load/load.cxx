...
try
{
  ...
  else
  {
    if (ops.tenant_load ())
    {
      cerr << "error: --tenant-load requires --tenant" << endl
           << help_info << endl;
      throw failed ();
    }
  }
  ...
  {
    shared_ptr<tenant> t; // Not NULL in the --tenant-load mode.
    ...
    // and, unless in the --tenant-load mode, the specified one.
    //
    ...
    else                             // Multi-tenant mode.
    {
      ...
      // In the --tenant-load mode make sure that the specified tenant exists,
      // is not archived, not marked as unloaded, and is empty. Otherwise (not
      // in the --tenant-load mode), remove this tenant.
      //
      if (ops.tenant_load ())
      {
    ...
    //
    // In the --tenant-load mode make sure that the specified service matches
    // the service associated with the pre-created tenant and update the
    // service data, if specified.
    //
